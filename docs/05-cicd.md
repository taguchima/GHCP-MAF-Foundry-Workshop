...
Lab 2 と同じく、Copilot Chat の chatmode picker で **`af-implementer`** に切り替えます（既存コードへの機能追加なので新たな設計判断は不要、`af-architect` を経由する必要はありません）。以下を入力：
...
`af-implementer` は [kb-1.8.0/api-reference/1.8.0/tools-mcp.md](../kb-1.8.0/api-reference/1.8.0/tools-mcp.md) を参照し、2 つ目の `MCPStreamableHTTPTool` を作って `tools=[mrc_mcp, learn_mcp]` に渡す形に書き換えます（Lab 2 と同じ canonical pattern を維持）。完成イメージ：

```python
# src/agent.py (抜粋)
from azure.identity.aio import AzureCliCredential  # ← async 版 (Lab 2 と同じ)
...
async def main() -> None:
    # MCP ツールは plain object として作り tools= に渡す。
    # agent の async with (client.as_agent) が自動で enter/exit する。
    ...
    async with AzureCliCredential() as credential:
        ...
        async with client.as_agent(
            ...
            # ... 既存の対話ループ（run_once / run_interactive）はそのまま
```

> [!IMPORTANT]
> Lab 2 の canonical pattern を維持してください。`Agent(client=FoundryChatClient(...))` や `async with FoundryChatClient(...)`、sync 版の `AzureCliCredential()` に書き換えられていないか確認します（詳細は [Lab 2](02-maf-agent.md) と [`missing-async-with-cleanup.md`](../kb-1.8.0/anti-patterns/missing-async-with-cleanup.md)）。

Lab 3 でデプロイ済みの `agent/main.py` も同様に Learn MCP を追加します。Hosted Agent では `MCPStreamableHTTPTool` ではなく `client.get_mcp_tool(...)` を使う点に注意してください（[kb-1.8.0/api-reference/1.8.0/tools-mcp.md の推論ルール](../kb-1.8.0/api-reference/1.8.0/tools-mcp.md#ユーザー指示からの推論ルール) で Copilot が自動判別）。

````
agent/main.py にも同じく Microsoft Learn MCP を Hosted MCP (client.get_mcp_tool) として追加してください。
````
...
URL を開いて、`intent_resolution`（Lab 4 の 4-2-3 で設定した評価器）のスコアと各サンプルの判定理由を確認してください。
...
azd ai agent show     # 新 version が active であることを確認
```
...
Lab 4 の `src/evaluate.py` の `main()` の最後（`Result:` を表示した後）に以下を追記すれば、CI が品質ゲートになります。変数名は Lab 4 の `eval_definition` / `run` / `client`（`project_client.get_openai_client()`）にそのまま合わせています。

```python
    # main() 内、Result 表示の後に追記（インデントは main() のスコープに合わせる）
